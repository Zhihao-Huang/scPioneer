#' Wilcox method in Seurat FindMarkers
#' 
#' @export
DEG_Seurat_wilcox <- function(object, ident.1, ident.2, 
                       group.by = NULL, 
                       min.pct = 0, logfc.threshold = 0,  p.adjust.method = NULL,
                       verbose = T, ...) {
  DEG <- FindMarkers(object, ident.1 = ident.1, ident.2 = ident.2, 
                     group.by = group.by, test.use = 'wilcox', 
